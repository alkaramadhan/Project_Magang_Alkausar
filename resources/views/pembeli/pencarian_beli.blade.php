<head>
  ...
  <meta name="keywords" value="example, carousel, full, fluid, liquid, window, width" /> -->
    ...
    @if(isset($search))
    <div class="con">
    <center><h3> The Search results for your query <b> {{ $query }} </b> are: {{$search->count()}}</h3></center>
        @foreach($search as $pos)
        <div class="row">
            <div class="card">
                <img src="{{ asset('image/produk/'.$pos->gambar1) }}" class="card-img-top" alt="...">
                <a href="#" onclick="showsnake()"><i class="fas fa-heart" id="changeRed"></i></a>
                <div class="card-body">

                    <a href="">
                        <p class="nmbarang">{{ $pos->nama_produk }}</p>
                    </a>
                    <p class="hrgbarang">{{ $pos->harga_satuan }}</p>
                    <img src="{{ asset('image/home/officialtore.svg')}}" width="20px" class="cardemblm mr-2" alt="">
                    <a href="">
                    
                        <p class="nmtoko">{{$pos->user->name}}</p>

                        <form action="{{route('wishlist.store')}}" method="POST">
                            {{csrf_field()}}
                            <input name="user_id" type="hidden" value="{{Auth::user()->id}}" />
                            <input name="product_id" type="hidden" value="{{$pos->id}}" />
